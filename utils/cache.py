...
def load_seller_info_cache_for_user(user_id: int) -> Dict[str, Any] | None:
    """Загружает кэш информации о продавце для конкретного пользователя."""
    path = os.path.join(CACHE_DIR, f"seller_info_user_{user_id}.json")
    if not os.path.isfile(path):
        return None
    try:
