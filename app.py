...
# -----------------------
# Кэшируем дневную динамику заказов, рассчитанную в /api/fbw/planning/data,
# чтобы модалка "Динамика заказов" не вызывала повторные запросы к WB.
...
from models import db, User, Notification, PurchasePrice, FbwPlanningArchive, delete_user_with_related
db.init_app(app)
...
from blueprints.transit_routes import transit_routes_bp
...
def _resolve_fbw_supply_status(
    ...
    *,
    ...
    """Определяет текстовый статус FBW-поставки по ответу API (details + list item)."""
    it = list_item or {}
    ...
    if not status_name:
        if fact_date:
            ...
            try:
                ...
                if planned_dt:
                    planned_dt_msk = to_moscow(planned_dt)
                    if planned_dt_msk:
                        today = datetime.now(MOSCOW_TZ).date()
                        if planned_dt_msk.date() < today:
    ...
    if not status_name and allow_cached_fallback and cached_item:
        cached_status_val = str(cached_item.get("status", "")).strip()
        if cached_status_val:
            status_name = cached_status_val
    return str(status_name or "").strip()


def _is_fbw_supply_accepted(
    status_name: str,
    *,
    status_id: Any = None,
    fact_date: Any = None,
) -> bool:
    """True, если поставка уже принята на складе WB."""
    if fact_date:
        return True
    try:
        if status_id is not None and int(status_id) == 5:
            return True
    except (ValueError, TypeError):
        pass
    return "Принято" in (status_name or "")


def _patch_fbw_supply_cache_item(supply_id_str: str, updates: dict[str, Any]) -> None:
    """Обновляет одну поставку в лёгком кэше /fbw после свежей проверки статуса."""
    try:
        cached = load_fbw_supplies_cache() or {}
        items = list(cached.get("items") or [])
        for i, item in enumerate(items):
            if str(item.get("supply_id") or "") == supply_id_str:
...
def fetch_fbw_last_supplies(token: str, limit: int = 15) -> list[dict[str, Any]]:
    ...
    for it in base_list:
        ...
        supply_date = details.get("supplyDate") or it.get("supplyDate")
        fact_date = details.get("factDate") or it.get("factDate")
        status_name = _resolve_fbw_supply_status(
            details,
            it,
    ...
    *,
    ...
    for day in requested_days:
        ...
        if entry is None:
            days_to_fetch.append(day)
            continue
        # Для последних дней разрешаем регулярное освежение, т.к. WB может ретроспективно
        # обновлять статусы/отмены по уже прошедшим датам.
        try:
...
def _process_finance_data(raw: List[Dict[str, Any]], req_from: str, req_to: str, user_id: int = None) -> Dict[str, Any]:
    ...
    return_oper_values_lower = {"возврат", "сторно продаж", "корректный возврат"}

    def _norm_text(v: Any) -> str:
        return str(v or "").strip().lower()

    def _is_sale_doc(doc_name: str) -> bool:
        d = _norm_text(doc_name)
        return d == "продажа" or "продаж" in d

    def _is_return_doc(doc_name: str) -> bool:
        d = _norm_text(doc_name)
    ...
    for r in raw:
        ...
        try:
            ...
            pass
        try:
            oper_lc = _norm_text(r.get("supplier_oper_name"))
            if oper_lc in buyout_oper_values_lower:
        ...
        try:
            ...
            if oper_lc in buyout_oper_values_lower:
                wbr_plus += amt
            elif oper_lc in return_oper_values_lower:
                wbr_minus += amt
        except Exception:
            pass
        try:
            oper_lc = _norm_text(r.get("supplier_oper_name"))
            if oper_lc in return_oper_values_lower:
                total_returns += float(r.get("retail_price") or 0.0)
        except Exception:
            pass
        try:
            dt_name = _norm_text(r.get("doc_type_name"))
            acq_pct = float(r.get("acquiring_percent") or 0.0)
            afee = float(r.get("acquiring_fee") or 0.0)
            if _is_sale_doc(dt_name) and acq_pct > 0:
                total_acquiring += afee
            elif _is_return_doc(dt_name) and acq_pct > 0:
                total_acquiring -= afee
        except Exception:
            pass
        try:
        ...
        try:
            ...
            if oper_l == "добровольная компенсация при возврате" and _is_sale_doc(doc_l):
                x1 += pay_val
            if oper_l == "частичная компенсация брака" and _is_sale_doc(doc_l):
                x2 += pay_val
            if oper_l == "добровольная компенсация при возврате" and _is_return_doc(doc_l):
                x3 += pay_val
            if oper_l == "частичная компенсация брака" and _is_return_doc(doc_l):
                x4 += pay_val
            if oper_l == "компенсация подмененного товара" and _is_sale_doc(doc_l):
                x5 += pay_val
            if oper_l == "компенсация подмененного товара" and _is_return_doc(doc_l):
                x6 += pay_val
            if oper_l == "компенсация потерянного товара" and _is_sale_doc(doc_l):
                x7 += pay_val
            if oper_l == "компенсация потерянного товара" and _is_return_doc(doc_l):
                x8 += pay_val
            if oper_l == "продажа":
                k1 += pay_val
            ...
            if oper_l == "корректная продажа":
                k3 += pay_val
            if oper_l == "коррекция продаж" and _is_sale_doc(doc_l):
                k4 += pay_val
            if oper_l == "возврат":
                k5 += pay_val
            if oper_l == "сторно продаж":
                k6 += pay_val
            if oper_l == "коррекция продаж" and _is_return_doc(doc_l):
                k7 += pay_val
            if oper_l == "корректный возврат":
                k8 += pay_val
            if "коррект" in oper_l and "эквайр" in oper_l:
                k9 += pay_val
            if oper_l == "частичная компенсация брака" and _is_sale_doc(doc_l):
                u1 += pay_val
            if oper_l == "добровольная компенсация брака" and _is_sale_doc(doc_l):
                u2 += pay_val
            if oper_l == "частичная компенсация брака" and _is_return_doc(doc_l):
                u3 += pay_val
            if oper_l == "добровольная компенсация брака" and _is_return_doc(doc_l):
                u4 += pay_val
            if oper_l == "компенсация брака до возврата товара" and _is_sale_doc(doc_l):
                u5 += pay_val
            if oper_l == "компенсация брака до возврата товара" and _is_return_doc(doc_l):
                u6 += pay_val
            if oper_l == "добровольная компенсация подмены товара" and _is_sale_doc(doc_l):
                u7 += pay_val
            if oper_l == "добровольная компенсация подмены товара" and _is_return_doc(doc_l):
                u8 += pay_val
            if oper_l == "компенсация подмененного товара" and _is_sale_doc(doc_l):
                u9 += pay_val
            if oper_l == "компенсация подмененного товара" and _is_return_doc(doc_l):
                u10 += pay_val
            if oper_l == "добровольная компенсация утилизации товара" and _is_sale_doc(doc_l):
                u11 += pay_val
            if oper_l == "добровольная компенсация утилизации товара" and _is_return_doc(doc_l):
                u12 += pay_val
            if oper_l == "компенсация утилизации товара" and _is_sale_doc(doc_l):
                u13 += pay_val
            if oper_l == "компенсация утилизации товара" and _is_return_doc(doc_l):
                u14 += pay_val
        except Exception:
            pass
    ...
    for r in raw:
        try:
            oper_name = _norm_text(r.get("supplier_oper_name"))
            if "коррект" in oper_name and "эквайр" in oper_name:
...
def to_fbs_rows(orders: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for o in orders:
        if not isinstance(o, dict):
            ...
            continue
        # Номер задания: поддержка старого и нового формата WB
        order_id = (
            o.get("ID")
            or o.get("id")
            or o.get("orderId")
        ...
        ca_raw = (
            ...
            or o.get("orderCreatedAt")
            or o.get("orderCreateDate")
            or o.get("date")
        )
        ...
        # Цена: API может присылать разные поля и в разных единицах
        raw_price = (
            o.get("convertedPrice")
            or o.get("converted_price")
        ...
        try:
            pv = float(raw_price)
            price_value = int(pv // 100) if pv >= 1000 else int(round(pv))
        except Exception:
            price_value = 0
        ...
        if not warehouse:
            warehouse = (
                o.get("warehouseName")
                or o.get("warehouse")
                or o.get("warehouseId")
                or o.get("deliveryType")
                or o.get("supplyWarehouse")
                or ""
            )
        ...
        for format_idx, format_func in enumerate(body_formats):
            ...
            for attempt in range(3):  # Max 3 retries per format
                try:
                    ...
                    # Если WB возвращает 400 IncorrectRequestBody, прекращаем PUT
                    # для текущего формата payload и пробуем следующий.
                    if resp.status_code == 400:
                        ...
                        if attempt == 0:  # Only try without Bearer on first attempt
                            ...
                            if resp2.status_code == 204:
                                ...
                                break
                    # На этом обработка HTTP-ошибки завершена для текущего формата.
                    break
...
def _log_wb_notification_error(context: str, user_id: int, exc: BaseException) -> None:
    """
    Ошибки WB при фоновых уведомлениях: 401/403/429 — штатные ситуации,
    не засоряем лог полным traceback.
    """
    if isinstance(exc, requests.HTTPError) and exc.response is not None:
        code = exc.response.status_code
        if code in (401, 403):
            print(
                f"{context}: user_id={user_id} — WB вернул {code} "
                f"(токен недействителен, отозван или без доступа к API заказов FBS), проверка пропущена"
            )
            return
        if code == 429:
            print(f"{context}: user_id={user_id} — WB 429 (лимит запросов), проверка пропущена")
            return
        ...
        try:
            ...
            for user in users:
                try:
                    ...
                    _log_wb_notification_error("DBS уведомления (заказы)", user.id, e)
                    continue
                ...
                for user in active_users:
                    ...
                    if not existing_notification:
                        ...
                        create_notification(
                            ...
                            message=f"Доступна версия {current_version}",
                            notification_type="version_update",
...
def start_notification_monitoring():
    ...
    # Даже не активируя фоновые WB-потоки, держим время последнего запуска проверки,
    # чтобы не вызывать цикл чаще необходимого.
    ...
    try:
        ...
        with app.app_context():
            ...
            if not user or not user.wb_token:
                ...
                return 0
            if not user_may_call_wb_api(user):
                print(f"WB API skipped for user {user_id}: срок действия токена не в допустимом периоде или учётка неактивна")
                return 0
...
def _is_placeholder_org_name(name: str | None) -> bool:
    ...
    if not val:
        return True
    lowered = val.lower()
    if lowered in ("\u043f\u0440\u043e\u0444\u0438\u043b\u044c", "profile", "профиль"):
        return True
...
def _get_light_orders_cache_info(user_id: int) -> dict[str, Any] | None:
    """
    Lightweight info for orders cache on profile page.
    Uses warm meta as primary source, and file mtime fallback for last_updated.
    """
    meta = load_orders_cache_meta(user_id)
    period_path = _orders_period_cache_path_for_user(user_id)
    ...
    try:
        ...
        period_dt = None
    # If period cache is fresher than warm meta, show fresher time on profile.
    if period_dt and (not meta_dt or period_dt > meta_dt):
        last_updated = period_mtime_iso
    period_is_fresh = False
    if period_mtime_iso:
        try:
            period_is_fresh = (datetime.now(MOSCOW_TZ) - datetime.fromisoformat(period_mtime_iso)).total_seconds() < 24 * 3600
        except Exception:
            period_is_fresh = False
    ...
    return {
        ...
        "is_fresh": is_orders_cache_fresh(user_id) or period_is_fresh,
                        ...
                        if SUPPLIES_CACHE_AUTO and (token_changed or not is_supplies_cache_fresh()):
                            ...
                            def build_cache_background():
                                try:
                                    ...
                                    print(f"Кэш поставок успешно построен для пользователя {user_id}")
                                except Exception as e:
...
def api_fbw_warehouses():
    ...
    try:
        # API WB иногда принимает только Bearer, иногда raw token.
        # Также формат ответа может быть list или dict с items/warehouses.
        candidate_headers = (
        ...
        warehouses_data: list[dict[str, Any]] = []
        if response is not None:
            payload = response.json()
            if isinstance(payload, list):
                warehouses_data = payload
                ...
                warehouses_data = extracted if isinstance(extracted, list) else []
        else:
            # Не падаем с 502: пробуем fallback-источники ниже.
            print(f"WARNING: supplies-api warehouses unavailable: {last_exc}")

        warehouses = []
        print(f"DEBUG: Получено записей складов: {len(warehouses_data)}")
        ...
        for warehouse in warehouses_data:
            ...
            if warehouse_id and warehouse_name:
                warehouses.append({
                    "id": warehouse_id,
                    "name": warehouse_name,
                    "city": warehouse.get("city", "") or warehouse.get("settlement", ""),
                    "address": warehouse.get("address", "") or warehouse.get("fullAddress", ""),
                    "is_sorting_center": bool(
                        warehouse.get("isSortingCenter")
                        or warehouse.get("is_sorting_center")
                        or warehouse.get("sortingCenter")
                    )
                })

        # Fallback: если supplies-api вернул пусто, пробуем получить склады из FBS API.
        if not warehouses:
            try:
                fbs_warehouses = fetch_fbs_warehouses(token) or []
        ...
        # Дополнительный fallback: склады из локального кэша остатков пользователя.
        # Это помогает найти склады, которые уже встречались в данных остатков
        ...
        try:
            ...
            if isinstance(cached_stocks, dict):
                ...
                if isinstance(stock_items, list):
                    names_from_stocks: set[str] = set()
                    for it in stock_items:
                        if not isinstance(it, dict):
                            continue
                        nm = str(it.get("warehouse") or "").strip()
                        if nm:
                            names_from_stocks.add(nm)
                    # Сопоставляем с уже полученными WB-складами (по нормализованному имени)
                    def _norm_name(v: str) -> str:
                        return str(v or "").strip().lower().replace("ё", "е")
                    known_names = {_norm_name(str(w.get("name") or "")) for w in warehouses}
                    for nm in sorted(names_from_stocks):
                        if _norm_name(nm) in known_names:
                            continue
                        warehouses.append({
                            "id": f"name::{nm}",
                            "name": nm,
        ...
        # Удаляем возможные дубли и стабилизируем порядок
        uniq: dict[str, dict[str, Any]] = {}
        for w in warehouses:
            uniq[str(w.get("id"))] = w
        warehouses = sorted(uniq.values(), key=lambda x: str(x.get("name") or "").lower())

        print(f"DEBUG: Нормализовано складов: {len(warehouses)}")
        ...
        return jsonify({"error": "server_error", "message": f"Ошибка: {str(exc)}"}), 500
def _load_stocks_for_planning(token: str, user_id: int, max_cache_age_sec: int = 600) -> tuple[list, str]:
    """
    Загружает остатки для планирования поставки.
    При наличии кэша не блокирует HTTP-запрос: устаревший кэш обновляется в фоне.
    """
    ...
    if updated_at:
        try:
            cache_time = datetime.strptime(updated_at, "%d.%m.%Y %H:%M:%S")
            should_refresh = (datetime.now() - cache_time).total_seconds() >= max_cache_age_sec
        except Exception as e:
            print(f"Ошибка парсинга времени кэша: {e}")

    if cached_items and not should_refresh:
        print(f"=== ПЛАНИРОВАНИЕ ПОСТАВКИ: Используем кэшированные остатки ({len(cached_items)} товаров) ===")
        return cached_items, updated_at or datetime.now().strftime("%d.%m.%Y %H:%M:%S")

    if cached_items and should_refresh:
        print(
            f"=== ПЛАНИРОВАНИЕ ПОСТАВКИ: Кэш устарел ({updated_at}), "
            f"используем его и обновляем в фоне ({len(cached_items)} товаров) ==="
        )
        _start_stocks_update_bg(user_id, token)
        return cached_items, updated_at or datetime.now().strftime("%d.%m.%Y %H:%M:%S")

    print("=== ПЛАНИРОВАНИЕ ПОСТАВКИ: Кэш пуст, загружаем остатки синхронно ===")
    try:
        ...
        print("=== ПЛАНИРОВАНИЕ ПОСТАВКИ: Таймаут ожидания API остатков ===")
        raise
    except requests.HTTPError as e:
        if e.response and e.response.status_code == 429 and cached_items:
...
def api_fbw_planning_stocks():
    ...
    try:
        from datetime import datetime
        # Загружаем остатки без блокировки HTTP-запроса при устаревшем кэше
        try:
            stocks, _stocks_updated_at = _load_stocks_for_planning(token, current_user.id)
        except TimeoutError:
            return jsonify({
                "error": "stocks_busy",
                "message": "Обновление остатков уже выполняется. Попробуйте через минуту.",
            }), 503
        except requests.HTTPError as e:
        ...
        print("=== DEBUG: доступные склады в кэше остатков ===")
        ...
        for stock in stocks:
            ...
            if barcode:
                # Складываем остатки по штрихкоду
                if warehouse_id == "all":
                    ...
                    # Сопоставляем с выбранным складом (с учетом нормализации)
        ...
        if warehouse_id == "all":
            print(f"Загружены остатки по всем складам: {len(warehouse_stocks)}")
            return jsonify({
                "success": True,
            ...
            print(f"Загружены остатки для склада '{warehouse_name}': {len(warehouse_stocks)}")
            return jsonify({
                "success": True,
                "stocks": warehouse_stocks,
        ...
        print("=== Ошибка API в api_fbw_planning_stocks ===")
        print(f"HTTP Error: {http_err.response.status_code}")
        print(f"Response: {http_err.response.text}")
        return jsonify({"error": "api_error", "message": f"Ошибка API: {http_err.response.status_code}"}), 502
    except Exception as exc:
        print("=== Ошибка в api_fbw_planning_stocks ===")
...
def _normalize_barcode_key(value: Any) -> str:
    """Нормализует баркод к строковому ключу, совместимому с /tools/prices."""
...
def _calc_tools_prices_margin_map(
    ...
    """Считает маржинальность по тем же формулам, что и /tools/prices."""
    ...
    try:
        ...
        # Баркоды из текущего списка товаров (не тянем все закупки пользователя — это дорого при большой БД)
        ...
        # Закупочные цены пользователя только по нужным баркодам (чанками — лимит IN в SQLite)
        ...
        # Сначала пробуем взять сохраненные значения маржинальности из БД.
        # Это гарантирует совпадение цифр с /tools/prices.
        try:
...
def api_fbw_planning_data():
    ...
    try:
        ...
        # 2. Получаем остатки (не блокируем запрос при устаревшем кэше)
        print("=== ПЛАНИРОВАНИЕ ПОСТАВКИ: Загружаем остатки ===")
        try:
            stocks, stocks_updated_at = _load_stocks_for_planning(token, current_user.id)
        except TimeoutError:
            return jsonify({
                "error": "stocks_busy",
                "message": "Обновление остатков уже выполняется. Попробуйте через минуту.",
        ...
        # Группируем заказы по баркодам + считаем по дням для поиска аномалий
        ...
        # Период для дневной шкалы (включительно)
        try:
        ...
        for order in filtered_orders:
            ...
            # date в WB обычно есть; если вдруг нет — берём lastChangeDate как fallback (хуже, но лучше чем ничего)
        ...
        # Сохраняем дневные ряды в кэш (чтобы динамика в модалке не дергала WB повторно)
        try:
        ...
        # Аномалии: ищем единичные пики относительно "обычных" дневных продаж
        # Возвращаем только баркоды с флагом, чтобы не раздувать JSON.
        ...
        if total_days >= 7:
            for bc, day_map in (daily_by_barcode or {}).items():
                if not day_map:
                    continue
                # Собираем ряд по дням периода, чтобы корректно учитывать "нули"
                values: list[int] = []
                if period_start and period_end:
                    cur = period_start
                ...
                # - должен быть заметный отрыв от медианы
                # - и абсолютный порог (чтобы не помечать 2->4)
                # Пример: "обычно 1..5, но разово 10" -> помечаем.
                abs_min_spike = 10
                mult_spike = 2.0
                add_spike = 5.0
                ...
                # Находим дату максимума (первая, если несколько)
                max_day = None
        ...
        # Получаем время обновления остатков
        now_str = stocks_updated_at
        ...
        # Маржинальность из /tools/prices (расчет по тем же данным/настройкам пользователя)
...
def api_fbw_planning_orders_dynamics():
    """API: динамика заказов по дням для одного товара (по баркоду) в рамках склада и периода."""
    warehouse_id = request.args.get("warehouse_id")
    ...
    try:
        ...
        # 0) Быстрый путь: если планирование уже считалось, берем дневные значения из in-memory кэша
        try:
            uid = int(current_user.id) if current_user.is_authenticated else None
            if uid is not None:
                    ...
                    return jsonify(
                        {
                            ...
                            "warehouse_name": cached.get("warehouse_name", f"Склад {warehouse_id}"),
                            "barcode": barcode,
        ...
        # Заполняем ряд по всем дням периода (включительно)
        series: list[dict[str, object]] = []
        cur = start_dt.date()
        while cur <= end_dt.date():
        ...
        if e.response and e.response.status_code == 429:
            ...
            try:
                ...
                retry_s = 60
            return jsonify({"error": "rate_limit", "message": "Превышен лимит запросов к API Wildberries.", "retry_after": retry_s}), 429
        return jsonify({"error": "api_error", "message": str(e)}), 502
    except Exception as e:
        print(f"Ошибка получения динамики заказов: {e}")
        return jsonify({"error": "server_error", "message": str(e)}), 500
...
def api_fbw_planning_export_excel():
    ...
    try:
        ...
        headers = [
            ...
            'Остаток по всем складам', 'В пути на склад', 'Заказано за период', 'Маржинальность',
        ...
        for row, product in enumerate(products_to_export, 1):
            ...
            worksheet.write(row, 7, product.get('marginPct', ''), number_style)  # Маржинальность
            worksheet.write(row, 8, round(product.get('salesPerDay', 0), 2), number_style)  # Продаж в день
            worksheet.write(row, 9, round(product.get('requiredStock', 0)), number_style)  # Необходимый остаток
            worksheet.write(row, 10, round(product.get('turnover', 0), 1), number_style)  # Оборачиваемость
            worksheet.write(row, 11, round(product.get('toSupply', 0)), number_style)  # Поставить на склад
...
def api_fbw_planning_supplies():
    ...
    try:
        ...
        # Предварительная фильтрация: по кэшу страницы /fbw (если есть) + обязательно свежие поставки без кэша.
        # Раньше поставки без записи в load_fbw_supplies_cache() отбрасывались — тогда планирование видело
        # «поставку в пути» только после открытия /fbw. Детальный кэш профиля (supplies_by_date) агрегирован
        # по дням без привязки supply_id, для этой проверки он не подходит.
        ...
        for supply in supplies_list:
            ...
            if cached_item:
                ...
                # Нет строки в лёгком кэше /fbw — всё равно проверяем через API (склад/статус уточняются по деталям)
                uncached_candidates.append(supply)

        uncached_sorted = sorted(uncached_candidates, key=_planning_supply_create_ts, reverse=True)
        # Сначала самые новые поставки без кэша (часто как раз «в пути»), затем отфильтрованные из кэша
        ...
        print(
            f"Планирование поставок: к проверке {len(supplies_to_check)} шт. "
            f"(из кэша /fbw: {len(supplies_from_cache)}, без кэша: {len(uncached_candidates)}, всего в списке API: {len(supplies_list)})"
        )
        ...
        for supply in supplies_to_check:
            ...
            try:
                ...
                # Статус при планировании всегда проверяем через API — кэш может быть устаревшим.
                details = None
                details_status = ""
                warehouse_from_details = ""
                box_type_name = ""
                box_type_id = None
                fact_date = None
                ...
                try:
                    ...
                    warehouse_from_details = (details.get("warehouseName") or supply.get("warehouseName") or "").strip()
                    ...
                    details_status = _resolve_fbw_supply_status(details, supply)
                    box_type_name = (details.get("boxTypeName") or "").strip()
                    box_type_id = details.get("boxTypeID")
                    if not box_type_name and box_type_id is not None:
                        box_type_map = {1: "Без коробов", 2: "Короба"}
                        try:
                            box_type_name = box_type_map.get(int(box_type_id), str(box_type_id))
                        except (ValueError, TypeError):
                            box_type_name = str(box_type_id)

                    cached_status = (cached_item or {}).get("status", "").strip()
                    if cached_status and cached_status != details_status:
                        print(
                            f"Поставка {supply_id}: статус обновлён "
                            f"'{cached_status}' -> '{details_status}' (планирование)"
                        )

                    _patch_fbw_supply_cache_item(
                        supply_id_str,
                        {
                            "status": details_status,
                            "warehouse": warehouse_from_details,
                            "type": box_type_name or (cached_item or {}).get("type", ""),
                            "planned_date": _fmt_dt_moscow(details.get("supplyDate"), with_time=False)
                            if details.get("supplyDate")
                            else (cached_item or {}).get("planned_date", ""),
                            "fact_date": _fmt_dt_moscow(fact_date, with_time=True) if fact_date else "",
                            "total_goods": int(details.get("quantity", 0) or 0)
                            if details.get("quantity") is not None
                            else (cached_item or {}).get("total_goods"),
                        },
                    )
                    print(f"Поставка {supply_id}: актуальный статус с API — '{details_status}'")
                except requests.Timeout:
                    print(f"Поставка {supply_id}: таймаут при загрузке деталей, пропускаем (без устаревшего кэша)")
                    continue
                except Exception as e:
                    print(f"Поставка {supply_id}: ошибка загрузки деталей: {e}, пропускаем")
                    continue

                # КРИТИЧНО: принятые поставки не должны учитываться в планировании
                if _is_fbw_supply_accepted(details_status, status_id=status_id, fact_date=fact_date):
                    print(
                        f"Поставка {supply_id}: статус '{details_status}' - ПРИНЯТА, "
                        f"пропускаем (не учитываем в планировании)"
                    )
                    continue
...
def api_profile_organization_refresh():
    ...
    if not token_api:
        return jsonify({"success": False, "error": "Токен WB API отсутствует или недействителен"}), 400
    ...
    if now_ts - last_attempt < _SELLER_INFO_MIN_INTERVAL_SECONDS:
        ...
        return jsonify({"success": False, "error": f"Слишком часто. Повторите через {max(1, wait_sec)} сек."}), 429
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"Ошибка запроса к WB API: {exc}"}), 502
    ...
    try:
        ...
        return jsonify({"success": False, "error": f"Некорректный JSON: {resp.text[:300]}"}), 502

    if not isinstance(seller_info, dict):
        return jsonify({"success": False, "error": "WB API вернул пустой/невалидный ответ", "raw": seller_info}), 502
    ...
    if not organization_name:
        return jsonify({
            "success": False,
            "error": "В ответе WB API нет полей name/companyName/supplierName",
...
def profile_org_display_name():
    """Обновляет название организации для шапки (без запроса к WB API)."""
    val = (request.form.get("org_display_name") or "").strip()
    try:
        current_user.org_display_name = val or None
        db.session.commit()
        flash("Название организации сохранено" if val else "Название организации очищено")
    except Exception:
        db.session.rollback()
...
def api_report_orders():
    ...
    try:
        ...
        # В режиме "Показать весь товар" гарантируем, что каталог актуально подгружен,
        # даже если пользователь не открывал страницу /products.
        if show_all and token and not all_catalog_items:
            try:
                ...
                # Не прерываем формирование отчёта, если обновление каталога не удалось.
                pass
        # Load stocks data for current user
        stocks_by_warehouse = {}
        stocks_metadata = {}
        ...
        def _build_items(target_wh: str | None, show_all: bool = False) -> List[Dict[str, Any]]:
            ...
            all_products = set(counts_total.keys())
            if show_all:
                # Добавляем полный каталог товаров (из кэша /products), даже без продаж и остатков.
...
def fbs_export():
    ...
    try:
        # 1) Берём данные из кэша заданий (то, что реально видно на странице /fbs).
        # Это устраняет ситуацию, когда WB /new уже пустой, но кэш на странице ещё содержит задания.
        cached = load_fbs_tasks_cache() or {}
        rows = (cached.get("rows") or []) if isinstance(cached, dict) else []

        # 2) Если кэша нет — пробуем получить свежие данные (fallback).
        if not rows:
            raw = fetch_fbs_new_orders(token)
            raw_sorted = sorted(raw, key=_extract_created_at)
            rows = to_fbs_rows(raw_sorted)

            # Enrich from products cache (для баркодов/фото)
        ...
        # 3) Агрегация дублей: nm_id -> barcode -> name (устойчивый ключ)
...
def api_fbs_tasks():
    def _normalize_task_row(row: Dict[str, Any]) -> Dict[str, Any]:
        if not isinstance(row, dict):
            return {
                "Номер и дата задания": str(row or ""),
                "Наименование товара": "",
                "Стоимость товара": 0,
                "Цена": 0,
                "Склад": "",
        ...
        num_date = row.get("Номер и дата задания") or row.get("Номер и дата заказа") or ""
        name = row.get("Наименование товара") or row.get("Наименование") or row.get("article") or ""
        price = row.get("Цена")
        if price is None:
            price = row.get("Стоимость товара")
        if price is None:
            price = row.get("Цена с учетом всех скидок")
        if price is None:
            price = row.get("Стоимость товара")
        warehouse = row.get("Склад")
        if warehouse is None:
            warehouse = row.get("Склад отгрузки")
        order_id = row.get("orderId") or row.get("ID") or row.get("id")
        return {
            **row,
            "Номер и дата задания": num_date,
            "Наименование товара": name,
            "Стоимость товара": price if price is not None else 0,
            "Цена": price if price is not None else 0,
            "Склад": warehouse or "",
            "orderId": order_id,
        }
    ...
    try:
        if not refresh:
            cached = load_fbs_tasks_cache() or {}
            if cached.get("rows"):
                normalized_rows = [_normalize_task_row(r) for r in (cached.get("rows") or [])]
                return jsonify({"items": normalized_rows, "updated_at": cached.get("updated_at")})
        # Fetch fresh
        print(f"=== FETCHING FRESH FBS TASKS ===")
        raw = fetch_fbs_new_orders(token)
        raw_items, _ = _normalize_fbs_orders_page(raw)
        print(f"Raw tasks count: {len(raw_items)}")
        raw_sorted = sorted(raw_items, key=_extract_created_at)
        rows = to_fbs_rows(raw_sorted)
        ...
        for it in items:
            ...
            if nm:
                try:
                    ...
                    pass
        for r in rows:
            art = (r.get("Наименование товара") or r.get("article") or "").strip()
            hit = by_article.get(art)
            if not hit and r.get("nm_id"):
                try:
                    hit = by_nm.get(int(r["nm_id"]))
                    ...
                    hit = None
            if hit:
                # В свежем API WB поле article может отсутствовать — подставляем
                # человекочитаемое имя из кэша товаров.
                if not (r.get("Наименование товара") or "").strip():
                    r["Наименование товара"] = (
                        hit.get("name")
                        or hit.get("title")
                        or hit.get("imt_name")
                        or hit.get("subject_name")
...
def fetch_stocks_resilient(token: str, lock_timeout: float | None = 120.0) -> List[Dict[str, Any]]:
    global _last_stocks_request_time

    if lock_timeout is not None:
        acquired = _stocks_api_lock.acquire(timeout=lock_timeout)
        if not acquired:
            raise TimeoutError(
                "Таймаут ожидания доступа к API остатков: другой запрос уже выполняется"
            )
    else:
...
def _start_stocks_update_bg(user_id: int, token: str) -> None:
    """Запускает фоновое обновление остатков для пользователя."""
    if not token:
        return
...
def _banner_visible_today(cookie_name: str, today) -> bool:
    """False если пользователь скрыл баннер cookie на сегодня или позже (как у подписки)."""
...
def _init_db_once_per_process():
    ...
    if _DB_INIT_DONE:
        # На случай добавления новых моделей/таблиц в уже запущенном процессе:
        # проверяем наличие таблицы маржинальности и создаём при необходимости.
        try:
...
def api_notifications():
    ...
    def _is_garbled_text(text: str) -> bool:
        ...
        if not val:
            return True
        q_ratio = val.count("?") / max(1, len(val))
        has_letters = any(ch.isalpha() for ch in val)
        return q_ratio >= 0.4 and not has_letters

    def _fallback_notification_text(notif_type: str, data: dict | None) -> tuple[str, str]:
        payload = data if isinstance(data, dict) else {}
        order_id = str(payload.get("order_id") or "").strip()
        version = str(payload.get("version") or "").strip()
        if notif_type == "fbs_new_order":
            suffix = f" #{order_id}" if order_id else ""
            return "Новый заказ FBS", f"Поступил новый заказ{suffix}"
        if notif_type == "dbs_new_order":
            suffix = f" #{order_id}" if order_id else ""
            return "Новый заказ DBS", f"Поступил новый заказ DBS{suffix}"
        if notif_type == "version_update":
            if version:
                return "Обновление версии", f"Доступна версия {version}"
            return "Обновление версии", "Доступна новая версия"
        if notif_type == "test":
            return "Тестовое уведомление", "Проверка системы уведомлений"
        return "Уведомление", "Новое событие"
    ...
    try:
        ...
        for notif in notifications:
            ...
            # Если время уже с timezone, конвертируем в московское
                moscow_time = created_at.replace(tzinfo=MOSCOW_TZ)
            else:
            # Если время уже с timezone, конвертируем в московское
            ...
            if _is_garbled_text(title) or _is_garbled_text(message):
                title, message = _fallback_notification_text(notif.notification_type, data)
        ...
        app.logger.exception("api_notifications failed: %s", exc)
        return jsonify({"notifications": [], "error": "failed_to_load_notifications"}), 200
    ...
    if not token:
        ...
        try:
            ...
            try:
                ...
                if purchase_prices:
                    print(f"Примеры загруженных баркодов из БД: {list(purchase_prices.keys())[:5]}")
                if products:
...
def api_tools_prices_margins_save():
    """Сохранение маржинальности (profit_pct/profit_net) из /tools/prices в БД."""
    try:
...
def _serialize_fbw_plan(plan: FbwPlanningArchive) -> dict[str, Any]:
    return {
        "id": plan.id,
        "date": (plan.updated_at or plan.created_at).isoformat() if (plan.updated_at or plan.created_at) else "",
        "warehouse_id": plan.warehouse_id or "",
        "warehouse_name": plan.warehouse_name or "",
        "sales_period_from": plan.sales_period_from or "",
        "sales_period_to": plan.sales_period_to or "",
        "planning_days": plan.planning_days,
        "supply_date": plan.supply_date or "",
        "total_current_stock": int(plan.total_current_stock or 0),
        "total_to_supply": int(plan.total_to_supply or 0),
    }


@app.route("/api/fbw/planning/archive", methods=["GET"])
@login_required
def api_fbw_planning_archive():
    plans = (
        FbwPlanningArchive.query
        .filter_by(user_id=current_user.id)
        .order_by(FbwPlanningArchive.updated_at.desc(), FbwPlanningArchive.id.desc())
        .all()
    )
    return jsonify({"success": True, "plans": [_serialize_fbw_plan(p) for p in plans]})


@app.route("/api/fbw/planning/archive/<int:plan_id>", methods=["GET"])
@login_required
def api_fbw_planning_archive_detail(plan_id: int):
    plan = FbwPlanningArchive.query.filter_by(id=plan_id, user_id=current_user.id).first()
    if not plan:
        return jsonify({"success": False, "message": "Планирование не найдено"}), 404

    try:
        products = json.loads(plan.products_json or "[]")
    except Exception:
        products = []

    return jsonify({
        "success": True,
        "plan": _serialize_fbw_plan(plan),
        "products": products if isinstance(products, list) else [],
    })


@app.route("/api/fbw/planning/archive/<int:plan_id>", methods=["DELETE"])
@login_required
def api_fbw_planning_archive_delete(plan_id: int):
    plan = FbwPlanningArchive.query.filter_by(id=plan_id, user_id=current_user.id).first()
    if not plan:
        return jsonify({"success": False, "message": "Планирование не найдено"}), 404
...
@app.route("/api/fbw/planning/archive/save", methods=["POST"])
@login_required
def api_fbw_planning_archive_save():
    payload = request.get_json(silent=True) or {}
    ...
    if not isinstance(form_data, dict):
        return jsonify({"success": False, "message": "Некорректные параметры формы"}), 400
    if not isinstance(products, list) or not products:
        return jsonify({"success": False, "message": "Нет данных для сохранения"}), 400

    warehouse_name = str(form_data.get("warehouse_name") or "").strip()
    if not warehouse_name:
        warehouse_name = str(form_data.get("warehouse") or "").strip()
    if not warehouse_name:
        return jsonify({"success": False, "message": "Не указан склад назначения"}), 400
    ...
    for item in products:
        if not isinstance(item, dict):
            continue
        current_stock = int(round(float(item.get("currentStock") or 0)))
        to_supply = int(round(float(item.get("toSupply") or 0)))
        if to_supply < 0:
            to_supply = 0
        total_current_stock += current_stock
        total_to_supply += to_supply
        normalized_products.append(
            {
                ...
                "allStocks": int(round(float(item.get("allStocks") or 0))),
                "inTransit": int(round(float(item.get("inTransit") or 0))),
                "orderedInPeriod": int(round(float(item.get("orderedInPeriod") or 0))),
            }
        )

    if not normalized_products:
        return jsonify({"success": False, "message": "Нет валидных товаров для сохранения"}), 400

    plan: FbwPlanningArchive | None = None
    ...
    if not plan:
        plan = FbwPlanningArchive(user_id=current_user.id)
        db.session.add(plan)

    plan.warehouse_id = str(form_data.get("warehouse") or "")
    plan.warehouse_name = warehouse_name
    plan.sales_period_from = str(form_data.get("salesPeriodFrom") or "")
    plan.sales_period_to = str(form_data.get("salesPeriodTo") or "")
    plan.planning_days = int(form_data.get("planningDays") or 0) if str(form_data.get("planningDays") or "").strip() else None
    plan.supply_date = str(form_data.get("supplyDate") or "")
    plan.total_current_stock = total_current_stock
    plan.total_to_supply = total_to_supply
    plan.products_json = json.dumps(normalized_products, ensure_ascii=False)
    plan.updated_at = datetime.now(MOSCOW_TZ)

    db.session.commit()
    return jsonify({"success": True, "plan": _serialize_fbw_plan(plan)})
